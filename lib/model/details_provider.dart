...
class DetailsProvider extends ChangeNotifier {
  ...
  Future<void> downloadFile(
    ...
    try {
      ...
      debugPrint("DEBUG PROVIDER: URL para download: $url");
      debugPrint("DEBUG PROVIDER: Extensão detectada: $fileExtension");

      String path = Platform.isIOS
          ? '${appDocDir!.path}/$filename$fileExtension'
          : '${appDocDir!.path}/Ebook/$filename$fileExtension';

      debugPrint("DEBUG PROVIDER: Caminho de destino: $path");

      // Garantir que o diretório pai exista
      Directory parent = Directory(File(path).parent.path);
      if (!await parent.exists()) {
        await parent.create(recursive: true);
        debugPrint("DEBUG PROVIDER: Diretório pai criado: ${parent.path}");
      }
      ...
      if (!await file.exists()) {
        await file.create();
        debugPrint("DEBUG PROVIDER: Arquivo criado: $path");
      } else {
        await file.delete();
        await file.create();
        debugPrint("DEBUG PROVIDER: Arquivo substituído: $path");
      }
      ...
      final result = await showDialog(
        ...
        builder: (dialogContext) =>
            DownloadAlert(url: url, path: path, id: id),
      );
      ...
      if (result != null) {
        ...
        debugPrint(
          "DEBUG PROVIDER: Download concluído. Verificando arquivo...",
        );
        ...
        if (await file.exists()) {
          ...
          debugPrint(
            "DEBUG PROVIDER: Arquivo existe. Tamanho: $fileSize bytes",
          );

          if (fileSize > 0) {
            debugPrint("DEBUG PROVIDER: Download bem-sucedido!");
          } else {
            debugPrint(
              "DEBUG PROVIDER: Arquivo foi criado mas está vazio!",
            );
          }
        } else {
          debugPrint(
            "DEBUG PROVIDER: Arquivo não foi encontrado após o download!",
          );
        }
      ...
      debugPrint('DEBUG PROVIDER: DioException $e');
      debugPrint('DEBUG PROVIDER: DioException message: ${e.message}');
      down.value = false;
    } catch (e) {
      debugPrint('DEBUG PROVIDER: Erro geral: $e');
      down.value = false;
    }
