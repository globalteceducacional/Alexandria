...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  Future<String?> _getValidFilePath(String basePath) async {
    ...
    if (await file.exists() && await file.length() > 0) {
      if (kDebugMode) {
        debugPrint("Encontrado arquivo válido: ${file.path}");
      }
      return file.path;
    }
    ...
    try {
      ...
      if (isDownloaded) {
        ...
        if (downloadModel.isNotEmpty) {
          ...
          if (kDebugMode) {
            debugPrint("Caminho encontrado no banco: $path");
          }
          ...
          debugPrint("Registro encontrado mas sem dados válidos");
        }
      } else if (kDebugMode) {
        debugPrint(
      ...
      if (kDebugMode) {
        debugPrint("Erro ao verificar arquivo local: $e");
      }
      return false;
    }
  }
