...
import 'package:elearn/model/details_provider.dart';
...
class _DetailsScreenState extends State<DetailsScreen> {
      ...
      if (Platform.isAndroid) {
        ...
        if (appDocDir != null) {
          ...
          if (!await ebookDir.exists()) {
            ...
            debugPrint(
                "DEBUG: Diretório para e-books criado em: ${ebookDir.path}");
          } else {
            debugPrint(
            ...
            debugPrint(
                "DEBUG: ${files.length} arquivos encontrados no diretório");
            for (var file in files) {
              debugPrint("DEBUG: Arquivo encontrado: ${file.path}");
            }
          }
        }
        ...
        debugPrint("DEBUG: Diretório do iOS para e-books: ${appDocDir.path}");

        // Lista os arquivos no diretório para debug
        List<FileSystemEntity> files = await appDocDir.list().toList();
        debugPrint(
            "DEBUG: ${files.length} arquivos encontrados no diretório iOS");
        for (var file in files) {
          debugPrint("DEBUG: Arquivo encontrado: ${file.path}");
        }
      }
    } catch (e) {
      debugPrint("DEBUG: Erro ao criar diretórios: $e");
    }
  }
  ...
  Future<bool> checkPhysicalBook() async {
    try {
      ...
      if (!dbResult) {
        ...
        if (filePath != null) {
          debugPrint("DEBUG: Livro encontrado fora do banco em: $filePath");
      ...
      debugPrint("DEBUG: Erro ao verificar arquivo físico: $e");
      return false;
    }
  }
  ...
  Future<String?> findBookFile() async {
    try {
      ...
      debugPrint("DEBUG: Erro ao procurar arquivo do livro: $e");
      return null;
    }
  }
  ...
  Future getDatabase() async {
    try {
      ...
      debugPrint("DEBUG: getDatabase() - Livro ID: ${widget.id}");
      debugPrint("DEBUG: getDatabase() - Favorito: $fav");
      debugPrint("DEBUG: getDatabase() - Baixado: $read");

      return;
    } catch (e) {
      debugPrint("DEBUG: Erro ao consultar banco de dados: $e");
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          StarRating(
            ...
            starSize: 28,
            filledColor: const Color(0xFFFFD700),
  ...
  MaterialColor getPrimaryAccentColor() {
    return const MaterialColor(0xff2055AD, {
      50: Color(0xffE4E9F2),
        ...
        children: [
                          ...
                          onTap: () async {
                            if (userId != null) {
                              if (singleBookById != null &&
                                ...
                                debugPrint("DEBUG: Caminho do livro: $path");
                                debugPrint(
                                    "DEBUG: Extensão detectada: $fileExtension");
                                debugPrint("DEBUG: URL original: $fileUrl");
                                ...
                                if (read) {
                                  ...
                                  down.value = false;
                                  debugPrint(
                                      "DEBUG: Iniciando download do livro");
          ...
          if (singleBookById != null && singleBookById!.ebookApp.isNotEmpty)
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.star, color: const Color(0xFFFFD700), size: 18.r),
                  SizedBox(width: 5.w),
                  Text(
    ...
    try {
      ...
      if (filePath != null) {
        ...
        if (!dbHasRecord) {
          ...
          debugPrint("DEBUG: Registro de download criado no banco de dados");
        }

        if (mounted) {
          setState(() {
            debugPrint("DEBUG: Estado de leitura atualizado para: $read");
          });
        }
      } else {
        debugPrint(
            "DEBUG: Arquivo não encontrado, livro não pode ser lido");
        read = false;
        if (mounted) {
          setState(() {});
        }
      ...
      debugPrint("DEBUG: Erro ao atualizar status de leitura: $e");
    }
  }
}
